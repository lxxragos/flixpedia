...
public class AdminPager {
	...
	public static String makeTag(HttpServletRequest request , int pageSize , int total) {
		...
		//pageSize - 한 페이지당 출력할 데이터 갯수
		//total - 전체 데이터 갯수
		//페이지 숫자 - total/pageSize		12/10 -> 강제올림 함수 ceil(1.2) -> 2
		...
		int cpage; //현재페이지 정보 - board/list?pg=1	이때 pg 값을 cpage로 처리하겠다. 
		// 12/10 -> ceiling(1.2) -> 2   
		int pageTotal; //전체 페이지 개수 
		int pageGroupSize = 5; //그룹의 개수 - 한 화면에 나타낼 페이지 최대개수 
