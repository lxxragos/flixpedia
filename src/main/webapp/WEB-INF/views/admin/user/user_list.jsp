...
<head>
    ...
    <title>FLIXMEDIA-user list</title>
                        ...
                        <div class="container" style="margin-top:80px">
                            ...
                            <div class="input-group mb-3" style="margin-top:20px;">
                                ...
                                <ul class="dropdown-menu">
                                  ...
                                  <li><a class="dropdown-item" href="#" onclick="changeSearch('2')">아이디</a></li>
                                  <li><a class="dropdown-item" href="#" onclick="changeSearch('3')">이름</a></li>
                                  <li><a class="dropdown-item" href="#" onclick="changeSearch('4')">닉네임</a></li>
                                </ul>
                            ...
                            <table class="table table-hover ">
                                ...
                                List<UserDto> list = (List<UserDto>)request.getAttribute("userList");
                                                                for(UserDto tempDto : list){
                                %>
                                  <tr>
                                    <td><%=totalCnt - tempDto.getRnum()+1%></td>
                                    <td><%=tempDto.getUser_id()%></td>
                                    <td><a href="#none" onclick="goView('<%=tempDto.getUser_seq()%>')"><%=tempDto.getName()%></a></td>
                                    <td><%=tempDto.getNick_name()%></td>
                                    <td><%=tempDto.getEmail()%></td>
                                    <td><%=tempDto.getPhone()%></td>
                                    <td><%=tempDto.getAddress1()%></td>
                                  </tr>
                                <%} %>
                                </tbody>
                              </table>
...
<script>
	window.onload = function(){
		let key = '<%=key%>';
		var texts=["","전체","아이디","이름","닉네임"];
		document.getElementById("searchItem").innerHTML=texts[key];	
	}
	function changeSearch(user_seq)
	{
		var texts=["","전체","아이디","이름","닉네임"];
		document.getElementById("searchItem").innerHTML=texts[user_seq]; //화면에 보이기 위해서
		document.getElementById("key").value=user_seq; //컨트롤러로 넘기기 위해서
	...
	function goPage(pg){
		...
		frm.pg.value = pg;
		frm.method = "get";
		frm.action = "${pageContext.request.contextPath}/admin/user/list";
		frm.submit();
	}
	
	function goView(user_seq){
		let frm = document.myform;
		frm.user_seq.value = user_seq;
		frm.method = "get";
		frm.action = "<%=request.getContextPath()%>/admin/user/view";
		frm.submit();
	}
	function goMain()
	{
